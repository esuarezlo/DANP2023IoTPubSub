...
public class BasicDiscovery {
    ...
    public void mainConnect(Context context) {
...
/*        final File rootCaFile = new File(cmdData.input_ca);
        ...
        // ----------------------------
        FileHelperEx fileHelperEx = new FileHelperEx(context);
        String input_cert = fileHelperEx.getContentString("xxxx.pem.crt");
        String input_key = fileHelperEx.getContentString("xxxx.pem.key");
        String input_ca = fileHelperEx.getContentString("xxxx.pem");

        try (final TlsContextOptions tlsCtxOptions = TlsContextOptions
                .createWithMtls(input_cert, input_key)) {
            //.createWithMtlsFromPath(cmdData.input_cert, cmdData.input_key)) {
            ...
            if (input_ca != null) {
                tlsCtxOptions
                        .overrideDefaultTrustStore(input_ca);
                //.overrideDefaultTrustStoreFromPath(null, cmdData.input_ca);
            }
/*            HttpProxyOptions proxyOptions = null;
            if (input_proxyHost != null && cmdData.input_proxyPort > 0) {
